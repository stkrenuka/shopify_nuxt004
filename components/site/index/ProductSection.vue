...
<template>
  <section id=""
    ...
    <div class="lg:w-[56.333%] w-full flex items-start lg:justify-start justify-center">
      <NuxtImg id="mainImage" :src="image" alt="Product Image" loading="lazy" height="500" width="500" quality="50"
        format="webp" class="w-full object-cover mb-8 transition-all duration-500" />
    </div>
    <div class="lg:w-[41.666%] w-full items-center lg:justify-end justify-center  lg:pl-3">
        ...
        <div v-for="option in product.options" :key="option.id">
          ...
          <div class="flex flex-wrap justify-center gap-2">
            <div class="text-center py-0 mb-3" v-if="option">
              ...
              <template v-else>
                <select class="text-md font-medium uppercase py-2 px-3 border border-gray-300 rounded"
                  @change="selectOption(option.name, $event.target.value)">
                  <option disabled value="">Select an option</option>
                  <option v-for="(value, index) in getValues(option.values)" :key="index" :value="value">
                    {{ value }}
                  </option>
                </select>
              </template>
      ...
      <div class="text-center py-4">
        <NuxtLink to="/products/10090278125842" class="text-[#8f51ac]">
